			...
			<div class="sidebar-scroll">
				<nav>
					<ul class="nav">
						...
						<li><a href="/yak_erp/dlistpage?num=1" class="active"><i class="lnr lnr-cog"></i> <span>콘텐츠관리</span></a></li>
						<li>
      ...
      <div class="main">
         ...
         <div class="main-content">
            <div class="container-fluid">
               <h3 class="page-title">콘텐츠 관리</h3>

                  <div class="col-md-12">
                     <!-- TABLE HOVER -->
                     <div class="panel" >
                        <div class="panel-heading">
                           <h3 class="panel-title">의약품 리스트</h3>
                  ...
                  <span class="input-group-btn"><button type="button" class="btn btn-primary" id="searchBtn">검색</button></span>
               </div><br/><br/>
                           <table class="table table-hover" style="width:80%; text-align:center;">
									<thead>
										<tr>
											<th style="width: 10%; text-align: center;">번호</th>
											<th style="width: 50%; text-align: center;">약품명</th>
											<th style="width: 50%; text-align: center;">제조사명</th>
										</tr>
									</thead>
									<c:forEach items="${list}" var="list">
										<tbody>
											<tr>
												<td>${list.rnum}</td>
												<td style = "cursor:pointer;"onClick="location.href='drug_view?drug_name=${list.drug_name}'">${list.drug_name}</td>
												<td>${list.d_company}</td>
								...
								<p class="demo-button" align=right>
									<button type="button" class="btn btn-default"
										onclick="location.href='drug_add'">약품 등록</button>

								</p>
								<div align="center">
											<c:if test="${page.prev}">
										<span>[ <a
											href="/yak_erp/dlistpage?num=${page.startPageNum - 1}${page.searchTypeKeyword}">이전</a>
											]
										</span>
									</c:if>
									...
									<c:forEach begin="${page.startPageNum}"
										...
										<span> <c:if test="${select != num}">
												<a
													href="/yak_erp/dlistpage?num=${num}${page.searchTypeKeyword}">${num}</a>
									...
									<c:if test="${page.next}">
										<span>[ <a
											href="/yak_erp/dlistpage?num=${page.endPageNum+1}${page.searchTypeKeyword}">다음</a>
											]
										</span>
									</c:if>
								</div>
   ...
   <script src="resources/scripts/klorofil-common.js"></script>
   	...
   	<script>
		document.getElementById("searchBtn").onclick = function() {
			...
			location.href = "/yak_erp/dlistpage?num=1" + "&keyword=" + keyword;
