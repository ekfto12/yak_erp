...
<body>
							...
							<div class="panel-body">
								<div class="input-group"
									...
									<input type="text" value="" class="form-control"
										placeholder="회원 검색" name="keyword"> <span
										class="input-group-btn"><button type="button" id="searchBtn"
								...
								<table class="table table-hover"
									...
									<c:forEach items="${list}" var="list">
										<tbody>
											<tr>
												<td>${list.rnum}</td>
												<td style = "color:blue; cursor:pointer;"onClick="location.href='Member_Information?member_id=${list.member_id}'">${list.member_id}</td>
								...
								<p class="demo-button" align=right>
									<button type="button" class="btn btn-default"
										onclick="location.href='Member_register'">회원 등록</button>

								</p>
								<div align="center">
									<c:if test="${page.prev}">
										<span>[ <a
											href="/yak_erp/mlistpage?num=${page.startPageNum - 1}${page.searchTypeKeyword}">이전</a>
											]
										</span>
									</c:if>
									...
									<c:forEach begin="${page.startPageNum}"
										...
										<span> <c:if test="${select != num}">
												<a
													href="/yak_erp/mlistpage?num=${num}${page.searchTypeKeyword}">${num}</a>
									...
									<c:if test="${page.next}">
										<span>[ <a
											href="/yak_erp/mlistpage?num=${page.endPageNum+1}${page.searchTypeKeyword}">다음</a>
											]
										</span>
									</c:if>
								</div>
	...
	<script>
		document.getElementById("searchBtn").onclick = function() {
			...
			location.href = "/yak_erp/mlistpage?num=1" + "&keyword=" + keyword;
